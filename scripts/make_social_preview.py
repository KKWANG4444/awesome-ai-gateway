...
GitHub-dark palette. Re-run after major README/star changes so the mini
comparison table on the card stays roughly in sync.

Data layer (TABLE, dimensions, palette) is kept importable WITHOUT Pillow so the
unit tests run on the CI runner (where Pillow is absent); the rendering path
(`render`) lazily imports PIL and is guarded under `__main__`.
"""
...
def render(out=OUT):
    """Render the card to `out` (imports PIL lazily — not available on CI)."""
